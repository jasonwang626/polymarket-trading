...
- [x] 有界、不可重用的資料收集工作階段；保存設定、停止原因、輪次／市場錯誤、資料稽核與檔案雜湊。
- [x] Mac 8 小時 session 驗收；依實測新增逐筆決策原因與乾淨 Git 來源狀態，避免把運作健康誤當成模型資料準備完成。
- [x] 完成使用者 Mac 十輪紀錄／SQLite 檢查，新增時間戳與深度診斷。
- [x] Mac 擴大至 10 個市場完成 360 輪；同步時鐘後另完成 30 輪。
- [x] HALTED 每 60 秒重新檢查、OPEN 恢復正常輪詢、未知狀態拒絕訊號。
- [ ] 新版暫停輪詢的 Mac 實測，以及市場恢復後的完整時段驗收。
...
本次完成基礎工作：
- [x] 唯讀 SQLite 自動稽核 CLI，輸出 JSON／Markdown 與明確限制。
- [x] 用 Mac 一小時及時鐘同步後資料重現資料庫統計。
- [x] 歷史外部價格同時限制來源時間與實際接收時間，加入延遲到達回歸測試。
- [x] 按已收到資料重播品質判斷，逐筆記錄所用快照與狀態／規則轉換。
- [x] 防止晚到價格、未來規則與未來接收委託簿影響過去判斷的回歸測試。
- [x] 重播兩份 Mac 資料，列明最近價格沿用政策與原始結果的差異。
- [x] 事件身分傳遞分組與時間切分清單，隔離跨界、embargo 與結果觀察截止洩漏。
- [x] 標籤準備度阻擋：未接入可信結果前，不產生可訓練 0/1 標籤。
- [x] 查核官方 settlement 端點及 BRTI 授權條件，完成公開結算證據收集與候選值驗證。
- [x] 收集結束自動封存可攜 SQLite、JSON／Markdown 健康報告；取消時也留下明確的不完整 session。
- [x] 30 天唯讀 campaign 編排：每日新分卷、乾淨版本與設定鎖定、磁碟預檢、雜湊事件鏈及中斷後新 attempt 安全續跑。
- [x] 使用者 Mac 24 小時正式 campaign 驗收：8,636 輪、0 失敗、clean Git provenance、封存與資料庫完整性通過。
- [ ] 真實已結算 BTC 樣本、特殊結算條款與結果可得時間驗收。
- [ ] 完整策略／執行重播及正式結算／BRTI 標籤驗證仍未完成。


1. 以已驗收的唯讀 campaign 連續收集 30 個每日分卷；先評估資料新鮮度、深度可用率與標籤可得性，再建立勝率基準。
